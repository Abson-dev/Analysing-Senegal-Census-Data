...
vars_to_remove = c("id_dr","REGION","DEPT","NOM_ARR","NOM_CA","RGPH","homme","hh_size","Aucun","Aucn_cm","Ltrcy_F","rsdnt_p","st_ccp_cc","st_ccp_rn","nbr_bbs","nbr_snr","MPI_men","cm_homm","pct_cm_h","geometry")


labels_var = c("Id dr","Region","Dept.","Nom Arr.","Nom CA","RGPH","Nombre d'homme","Nombre de femme","Taille ménage","Fertilité","Niveau d'instruction: Aucun","Niveau d'instruction: Primaire","Niveau d'instruction: Moyen","Niveau d'instruction: Secondaire", "Niveau d'instruction: Supérieur",
               
"Niveau d'instruction du CM: Aucun","Niveau d'instruction du CM: Primaire","Niveau d'instruction du CM: Moyen","Niveau d'instruction du CM: Secondaire", "Niveau d'instruction du CM: Supérieur","Nombre de French dans le ménage","Nombre de Arabic dans le ménage","Nombre de Wolof dans le ménage","Nombre de Pulaar dans le ménage","Nombre de Sereer dans le ménage","Nombre de Mandingo dans le ménage","Nombre de Diola dans le ménage","Nombre de Soninke dans le ménage","Nombre de résident permanent","Nombre de Résident Absent","Nombre de repas sauté","Nombre de soins médicaux",
               
"Nombre d'individus qui sont occupés dans leur occupation principale dans le DR","Nombre d'individus qui sont au chômage ou à la recherche d'un premier emploi dans leur occupation principale dans le DR","Nombre d'individus qui sont occupés au foyer dans leur occupation principale dans le DR","Nombre d'individus qui sont étudiants/élèves dans leur occupation principale dans le DR","Nombre d'individus qui sont rentiers dans leur occupation principale dans le DR","Nombre d'individus qui sont retraités ou personnes du 3ème âge dans leur occupation principale dans le DR","Nombre d'individus qui ont d'autres situations dans leur occupation principale dans le DR","Nombre d'enfant agée de moins d'un an dans le DR","Nombre d'enfant agée de moins de 14 ans dans le DR","Nombre d'individus agée entre 15 et 29 ans dans le DR","Nombre d'individus agées entre 30 et 44 ans dans le DR","Nombre d'individus agées entre 45 et 59 ans dans le DR","Nombre d'individus agée de plus de 60 ans dans le DR",
"Nombre de ménage dans le DR","Age médian du CM dans le DR","Age moyen du CM dans le DR","Nombre de CM de sexe masculin","Nombre de CM de sexe feminin","Proportion de CM de sexe masculin","Proportion de CM de sexe féminin",
...
label_to_remove = c("Id dr","Region","Dept.","Nom Arr.","Nom CA","RGPH","Nombre d'homme","Taille ménage","Taille ménage","Niveau d'instruction: Aucun","Niveau d'instruction du CM: Aucun","Nombre de French dans le ménage","Nombre de résident permanent","Nombre d'individus qui sont occupés dans leur occupation principale dans le DR","Nombre d'individus qui sont retraités ou personnes du 3ème âge dans leur occupation principale dans le DR","Nombre d'enfant agée de moins d'un an dans le DR","Nombre d'individus agée de plus de 60 ans dans le DR","Nombre de CM de sexe masculin","Proportion de CM de sexe masculin","geometry")
...
gtsummary::tbl_regression(model_2002,
                          include = everything(),
                          intercept = TRUE, show_single_row = everything(),
label = list(femme ~ "Nombre de femme" , fertlty ~ "Fertilité" , Primair ~ "Niveau d'instruction: Primaire" , Moyen ~ "Niveau d'instruction: Moyen" , Secondr ~ "Niveau d'instruction: Secondaire" , Superir ~ "Niveau d'instruction: Supérieur" , Prmr_cm ~ "Niveau d'instruction du CM: Primaire" , Moyn_cm ~ "Niveau d'instruction du CM: Moyen" , Scndr_c ~ "Niveau d'instruction du CM: Secondaire" , Sprr_cm ~ "Niveau d'instruction du CM: Supérieur" , Ltrcy_A ~ "Nombre de Arabic dans le ménage" , Ltrcy_W ~ "Nombre de Wolof dans le ménage" , Ltrcy_P ~ "Nombre de Pulaar dans le ménage" , Ltrcy_M ~ "Nombre de Sereer dans le ménage" , Ltrcy_D ~ "Nombre de Mandingo dans le ménage" , Ltrcy_S ~ "Nombre de Diola dans le ménage" , rsdnt_b ~ "Nombre de Soninke dans le ménage" , rsdnt_v ~ "Nombre de Résident Absent" , I_rps_s ~ "Nombre de repas sauté" , I_sns_m ~ "Nombre de soins médicaux" , st_ccp_ch ~ "Nombre d'individus qui sont au chômage ou à la recherche d'un premier emploi dans leur occupation principale dans le DR" , st_ccp_td ~ "Nombre d'individus qui sont occupés au foyer dans leur occupation principale dans le DR" , st_ccp_f ~ "Nombre d'individus qui sont étudiants/élèves dans leur occupation principale dans le DR" , st_ccp_rt ~ "Nombre d'individus qui sont rentiers dans leur occupation principale dans le DR" ,  st_ccp_tr ~ "Nombre d'individus qui ont d'autres situations dans leur occupation principale dans le DR" , nbr__14 ~ "Nombre d'enfant agée de moins de 14 ans dans le DR" , nbr_yn_ ~ "Nombre d'individus agée entre 15 et 29 ans dans le DR" , nbr_dlt ~ "Nombre d'individus agées entre 30 et 44 ans dans le DR" , nbr_ld_ ~ "Nombre d'individus agées entre 45 et 59 ans dans le DR" ,  menage ~ "Nombre de ménage dans le DR" , mdn_cm_ ~ "Age médian du CM dans le DR" , mn_cm_g ~ "Age moyen du CM dans le DR" , cm_femm ~ "Nombre de CM de sexe feminin" , pct_cm_f ~ "Proportion de CM de sexe féminin" , mn_nd_p ~ "Moyenne de l'indice de peoplement" , mdn_nd_ ~ "Médiane de l'indice de peoplement" , pop_density ~ "densité de la population")
...
# gtsummary::tbl_regression(model_2002,
#                      
#                            # label = list(age ~ "Age (in year)", 
#                            #              female ~ "Sex  of the target person (1 = Female, 0 = Male)", 
#                            #              male ~ "Sex  of the target person (0 = Female, 1 = Male)", 
#                            #              famstat ~ "Family status (1= Married or Cohabitation, 0= Otherwise)", 
#                            #              hh_size ~ "Household size",
#                            #              eduattain ~ "Educational attainment (0 = primary education or less; 1 = secondary education or more)", 
#                            #              contact_abroad_b ~ "Contacts with people abroad (1 =Yes, 0 = No)", 
#                            #              employed_b ~ "Any work in the last 30 days (1 =Yes, 0 = No)", 
#                            #              save_b ~ "Able to save every month (1 =Nothing, 0 = A little bit or a lot)", 
#                            #              hh_remit_b ~ "Receiving remittances (1 =Yes, 0 = No)",
#                            #              mig_before ~ "Previous migration experience  (1 =Yes, 0 = No)",
#                            #              know_return2_b ~ "Knowledge of a returnee irregular migrant (1= Yes, 0 = No)", 
#                            #              urban ~ "Living in urban area (1 = Urban or semi-urban, 0 = Rural)", 
#                            #              rural ~ "Living in rural area (1 = Rural, 0 = Urban or semi-urban)"),
#                            #  statistic = list(all_continuous() ~ "{mean}"
#                             digits = everything() ~ c(2,2),
#                             missing = "no") %>% 
#   
#   modify_header(label ~ "Covariates") %>%
#   add_difference() %>% modify_column_hide(c(ci, p.value)) #%>%
  # add_overall() %>%
  # add_n()%>% 
  # add_stat(fns = everything() ~ add_by_n) %>%
  # modify_header(starts_with("add_n_stat") ~ "**N**", stat_0 ~ "**All**", stat_1 ~ "**Control**", stat_2 ~ "**Treatment**") %>%
  # modify_table_body(~ .x %>%
