...
menage_2002 <- menage_2002 %>% 
  select(id_dr, ID_MENAGE, 71:ncol(menage_2002)) %>% 
  left_join(deces_2002,
            by = "ID_MENAGE")%>% 
  mutate(RGPH = "2002" %>% structure(label = "RGPH"))
```

```{r}
# Combine the "menage_2013" and "menage_2002" datasets into a single dataset
# This code uses the "plyr::rbind.fill" function to stack the two datasets on top of each other, 
# creating a unified dataset named "sn_census."

sn_census <- menage_2013 %>% 
  plyr::rbind.fill(menage_2002)

# Saving in SPSS format
haven::write_sav(sn_census,paste0(here::here(), "/output/output_data/sn_census.sav"))

# Saving in R format
save(sn_census,file=paste0(here::here(), "/output/output_data/sn_census.RData"))
```
