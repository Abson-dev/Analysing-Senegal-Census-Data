---
title: "Spatial analysis using the Senegal Census data"
...
```r
MPI_data_dr_2013$lag_MPI_ndx <- lag.listw(weights_2013, MPI_data_dr_2013$MPI_ndx)
MPI_data_dr_2002$lag_MPI_ndx <- lag.listw(weights_2002, MPI_data_dr_2002$MPI_ndx)
```


```r
ggplot(MPI_data_dr_2013, aes(x = MPI_ndx, y = lag_MPI_ndx)) + 
  geom_point(alpha = 0.3) + 
  geom_abline(color = "red") + 
  theme_minimal() + 
  labs(title = " Scatterplot of MPI relative to its spatial lag for Senegal census 2013",
       x = "MPI",
       y = "Spatial lag, MPI", 
       caption = "")
```
...
ggplot(MPI_data_dr_2002, aes(x = MPI_ndx, y = lag_MPI_ndx)) + 
  geom_point(alpha = 0.3) + 
  geom_abline(color = "red") + 
  theme_minimal() + 
  labs(title = " Scatterplot of MPI relative to its spatial lag for Senegal census 2002",
       x = "MPI",
       y = "Spatial lag, MPI", 
       caption = "")
```
...
```r
moran.test(MPI_data_dr_2013$MPI_ndx, weights_2013)
```
...
## 
## data:  MPI_data_dr_2013$MPI_ndx  
## weights: weights_2013    
## 
## Moran I statistic standard deviate = 14.676, p-value < 2.2e-16
## alternative hypothesis: greater
## sample estimates:
## Moran I statistic       Expectation          Variance 
##      0.4172959435     -0.0023041475      0.0008174259
```

```r
moran.test(MPI_data_dr_2002$MPI_ndx, weights_2002)
```
...
## sample estimates:
## Moran I statistic       Expectation          Variance 
##       0.445655972      -0.003745318       0.001336949
```
...
MPI_data_dr_2013$localG <- localG(MPI_data_dr_2013$MPI_ndx, localg_weights_2013)
MPI_data_dr_2013$localG <- as.numeric(MPI_data_dr_2013$localG)
#
localg_weights_2002 <- nb2listw(include.self(neighbors_2002))
MPI_data_dr_2002$localG <- localG(MPI_data_dr_2002$MPI_ndx, localg_weights_2002)
...
labs(title = "Map of local Gi* scores for Senegal Census 2013")
```
...
ggplot(MPI_data_dr_2002) + 
  ...
  theme_void() + 
  labs(fill = "Local Gi* statistic") +
labs(title = "Map of local Gi* scores for Senegal Census 2002")
```
...
ggplot(MPI_data_dr_2013) + 
  ...
  theme_void() +
labs(title = "Map of local Gi* scores with significant clusters highlighted for Senegal Census 2013")
```
...
ggplot(MPI_data_dr_2002) + 
  ...
  scale_fill_manual(values = c("red", "blue", "grey")) + 
  theme_void() +
labs(title = "Map of local Gi* scores with significant clusters highlighted for Senegal Census 2002")
```
...
MPI_data_dr_2013$scaled_MPI_ndx <- as.numeric(scale(MPI_data_dr_2013$MPI_ndx))
MPI_data_dr_2002$scaled_MPI_ndx <- as.numeric(scale(MPI_data_dr_2002$MPI_ndx))

dfw_lisa_2013 <- localmoran_perm(
  MPI_data_dr_2013$scaled_MPI_ndx, 
  weights_2013, 
  nsim = 999L, 
...
dfw_lisa_2013_df <- MPI_data_dr_2013 %>%
  select(id_dr, scaled_MPI_ndx) %>%
  mutate(lagged_estimate = lag.listw(weights_2013, scaled_MPI_ndx)) %>%
  bind_cols(dfw_lisa_2013)

##
dfw_lisa_2002 <- localmoran_perm(
  MPI_data_dr_2002$scaled_MPI_ndx, 
  weights_2002, 
  nsim = 999L, 
  alternative = "two.sided"
) %>%
...
dfw_lisa_2002_df <- MPI_data_dr_2002 %>%
  select(id_dr, scaled_MPI_ndx) %>%
  mutate(lagged_estimate = lag.listw(weights_2002, scaled_MPI_ndx)) %>%
...
dfw_lisa_2013_clusters <- dfw_lisa_2013_df %>%
  mutate(lisa_cluster = case_when(
    ...
    scaled_MPI_ndx > 0 & local_i > 0 ~ "High-high",
    scaled_MPI_ndx > 0 & local_i < 0 ~ "High-low",
    scaled_MPI_ndx < 0 & local_i > 0 ~ "Low-low",
    scaled_MPI_ndx < 0 & local_i < 0 ~ "Low-high"
  ))
...
ggplot(dfw_lisa_2013_clusters, aes(x = scaled_MPI_ndx, 
                              y = lagged_estimate,
                              fill = lisa_cluster)) + 
  ...
  labs(x = "MPI (z-score)",
       ...
       fill = "Cluster type") + 
  labs(title = "LISA quadrant scatterplot for Senegal Census 2013")
```
...
##
ggplot(dfw_lisa_2002_clusters, aes(x = scaled_MPI_ndx, 
                                   y = lagged_estimate,
                                   fill = lisa_cluster)) + 
  geom_point(color = "black", shape = 21, size = 2) + 
  theme_minimal() + 
  ...
  labs(x = "MPI (z-score)",
       y = "Spatial lag of MPI (z-score)",
       fill = "Cluster type") + 
  labs(title = "LISA quadrant scatterplot for Senegal Census 2002")
```
...
ggplot(dfw_lisa_2013_clusters, aes(fill = lisa_cluster)) + 
  labs(title = " LISA cluster map for Senegal Census 2013") +
  geom_sf(size = 0.1) + 
  theme_void() + 
  scale_fill_manual(values = color_values) + 
...
ggplot(dfw_lisa_2002_clusters, aes(fill = lisa_cluster)) + 
  labs(title = " LISA cluster map for Senegal Census 2002") +
  geom_sf(size = 0.1) + 
  theme_void() + 
  scale_fill_manual(values = color_values) + 
  labs(fill = "Cluster type")
