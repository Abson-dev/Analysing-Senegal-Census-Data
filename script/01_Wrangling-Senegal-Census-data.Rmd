...
menage_2002 <- menage_2002 %>% 
  left_join(
    ...
    individus_2002 %>% 
      ...
      mutate(niveau_instruction_cm = educ %>% structure(label=paste0(Hmisc::label(individus_2002$educ), " CM")),
...
menage_2013 <- menage_2013 %>% 
  left_join(
    ...
    individus_2013 %>% 
      ...
      mutate(niveau_instruction_cm = educ %>% structure(label=paste0(Hmisc::label(individus_2013$educ),"du CM")), 
...
menage_2002 <- menage_2002 %>% 
      mutate(
        Aucun_cm =ifelse(niveau_instruction_cm == 0, 1,0),
        Primaire_cm = ifelse(niveau_instruction_cm == 1, 1,0),
        Moyen_cm = ifelse(niveau_instruction_cm == 2, 1,0),
        Secondaire_cm = ifelse(niveau_instruction_cm == 3, 1,0),
        Superieur_cm = ifelse(niveau_instruction_cm == 4, 1,0)
      ) %>% 
      
  mutate(
        Aucun_cm = as.numeric(Aucun_cm)%>%
          structure(label = "CM a aucun niveau de d'instruction") ,
        Primaire_cm = as.numeric(Primaire_cm) %>%
          structure(label = "CM a un niveau d'instruction Primaire"),
        Moyen_cm = as.numeric(Moyen_cm) %>%
          structure(label = "CM a un niveau d'instruction Moyen"),
        Secondaire_cm = as.numeric(Secondaire_cm) %>%
          structure(label = "CM a un niveau d'instruction de Secondaire"),
        Superieur_cm = as.numeric(Superieur_cm) %>%
          structure(label = "CM a un niveau d'instruction Supérieur"))
...
educ_var = c("Aucun_cm","Primaire_cm","Moyen_cm","Secondaire_cm","Superieur_cm")
```
